...
from .database import (
    ...
    deactivate_optimization_from_db,
    deactivate_project_from_db,
)
from . import db
from .utility import (
...
def check_uploaded_filter(filter_definition_json):
    # Before this is even checked, the filter definition has to be checked for
    # general validity (e.g. correct keys)
    # Check that the sets of keys from filter_definition_json and the following key set match
    keys_set = {
        "azimAngleMax",
        "azimAngleMin",
        "azimAngleStep",
    ...
    if not keys_set <= set(filter_definition_json.keys()):
        return dict({"error": "Some keys are missing in the filter definition"})
...
def save_json():
    ...
    if np.size(targets) == 1:
        ...
        data_to_json["targets_polarization"] = (
            targets[1::no_columns_targets].astype(float).tolist()
        )
        data_to_json["targets_polar_angle"] = [
            [float(x), float(y)] if y != "" else float(x)
            for x, y in zip(
    ...
    # Try to read in the data from the current filter design to compare what has
    # changed before saving
    latest_job = select_latest_optimization(session.get("job_id"))
    calculation_keys = [
        "calculation_type",
        "polarization",
        "wavelengthMin",
    ...
    calculation_json = {
        key: data_to_json[key] for key in calculation_keys if key in data_to_json
    }
    calculation_json_old = {
        key: json.loads(latest_job.current_json).get(key) for key in calculation_keys
    }
    ...
    layer_json = {key: data_to_json[key] for key in layer_keys if key in data_to_json}
    layer_json_old = {
        key: json.loads(latest_job.current_json).get(key) for key in layer_keys
    }
    ...
    target_json = {key: data_to_json[key] for key in target_keys if key in data_to_json}
    target_json_old = {
        key: json.loads(latest_job.current_json).get(key) for key in target_keys
    }

    description = "Updated "
    if json.dumps(data_to_json) == latest_job.current_json:
        print("No changes detected")
        # Display a modal that there were no changes to the filter design so no need to save
        return jsonify({"error": "Cannot save: no changes to the filter detected"}), 400
    elif calculation_json != calculation_json_old:
        description += "calculation settings "
    elif layer_json != layer_json_old:
        description += "layers "
    elif target_json != target_json_old:
        description += "targets"
...
def select_version():
    # Extract if user wants to delete the version or load it
    action = request.form.get("action")

    # Extract datetime
    match = re.search(
        r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})\s?", request.form.get("version")
    )
    if match:
        selected_version_str = match.group(1)
        selected_version = datetime.strptime(selected_version_str, "%Y-%m-%d %H:%M:%S")
    else:
        # Handle the case where the datetime part is not found
    ...
    if action == "select":
        ...
        # Delete the optimization entry for the selected version
        deactivate_optimization_from_db(selected_version, session.get("job_id"))
        return redirect(url_for("stack_bp.stack"))
...
@stack_bp.route("/delete_project", methods=["POST"])
def delete_project():
    # Load in template
    filter_version = request.form.get("filter_version")
    ...
    # Delete filter project from database by datetime, name and username
    deactivate_project_from_db(filter_name, filter_date)

    # Now select the latest project and load it
    latest_optimization = select_latest_optimization(username=session.get("user_id"))

    # Now set the job_id to what was selected and reload the page
    session["job_id"] = latest_optimization.job_id
    session["filter_name"] = latest_optimization.filter_name

    return jsonify({"redirect": url_for("stack_bp.stack")})


@stack_bp.route("/duplicate_project", methods=["POST"])
def duplicate_project():
    ...
    # Retrieve the optimization entry for the selected version
    original_job = select_job_by_datetime_and_name(filter_date, filter_name)

    # Create a new job entry with the same attributes as the original
    new_job = Job(
        job_id=get_latest_job_id() + 1,
        username=original_job.username,
        filter_name=request.form.get("new_filter_name"),
        optimization_method=original_job.optimization_method,
        current_json=original_job.current_json,
        time_stamp=datetime.now(),
    ...
    session["job_id"] = new_job.job_id
    session["filter_name"] = new_job.filter_name
