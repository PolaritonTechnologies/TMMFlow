...
def start_optimization():
    # First of all, check if there is already a job with the current job_id
    job_id = session.get("job_id")
    rq_job_id = redis_conn.get(f"custom_job_id:{job_id}")

    if rq_job_id:
        try:
            job_status = rq.job.Job.fetch(
                rq_job_id.decode("utf-8"), connection=redis_conn
            ).get_status()
            ...
            if job_status in ["queued", "started", "deferred", "scheduled"]:
                return (
                    jsonify(
                        {
                            "error": "Current stack is already being optimized, please wait for it to finish!"
                        },
                    ),
                    400,
                )
    ...
    # Query the state of the available rq workers
    # We could have a customer that wants e.g. 5 workers, so we start 5 rq
    # workers with the customers team name. We should then default a job to the
    # customers queue (whith the workers that default to the customers team name).
    # Only if no workers specifically waiting for the team's queue are
    # unavailable, default back to the default queue.
    message = ""
    queue_name = session["team"]
    queue = rq.Queue(queue_name, connection=redis_conn)
    workers = rq.Worker.all(queue=queue, connection=redis_conn)

    available_workers = [
        True if worker.state == "idle" else False for worker in workers
    ]

    if len(workers) == 0:
        print("Team has no workers. Using default workers instead.")
        queue_name = "default"
        queue = rq.Queue(queue_name, connection=redis_conn)
        workers = rq.Worker.all(queue=queue, connection=redis_conn)

        available_default_workers = [
            True if worker.state == "idle" else False for worker in workers
        ]
        if not any(available_default_workers):
            message = "No default workers available. Your job has been queued."
            print("No default workers available. Queuing job into default queue.")

    elif not any(available_workers):
        print("All team workers are busy. Checking default queue instead.")
        ...
        if not any(available_default_workers):
            message = "No team or default workers available. Your job has been queued."
            print("Also no default workers available. Queuing job into team queue.")
            queue_name = session["team"]
        else:
            print("Default workers are available. Queuing job into default queue")
            queue_name = "default"
    ...
    job = q.enqueue(
        ...
        job_timeout="7d",  # Increase the timeout to be able to run long simulations
        # job_id=str(job_id),
    )
