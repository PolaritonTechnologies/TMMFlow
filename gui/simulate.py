...
@simulate_bp.route("/calculate_fields_and_plot_ajax", methods=["POST"])
def calculate_fields_and_plot_ajax():
    data = request.json
    ...
    my_filter.calculate_fields_ar_data(
        wavelengths,
        polar_angles=[float(data["angle"])],
        azimuthal_angles=[float(data["azimuthalAngle"])],
        target_type=target_type,
        polarization=polarization,
        z_resolution=float(data["stackResolution"]),
    )
    calculated_data_df = my_filter.stored_data[0]

    distance = calculated_data_df.columns.to_numpy()
    wavelengths = calculated_data_df.index.to_numpy()
    color_values = calculated_data_df.to_numpy()
...
def plot_xy_ajax():
    ...
    plot_data = {
        ...
        + ", s-pol: "
        + str(float(request_data["polarization"]))
        + ", Az: "
        + str(request_data["azimuthalAngle"])
        + "°",
    }
...
def extract_spectral_parameters(current_json, plotting_data):
    ...
    if np.size(target_passband_wavelength[target_no]) == 1:
        ...
        if target_passband_wavelength[target_no][1] > np.max(plotting_data["y"]):
            ...
            target_index_low = np.argmin(
                abs(
                    np.array(plotting_data["y"])
                    - target_passband_wavelength[target_no][0]
                )
            )

            target_index_high = np.argmin(
                abs(
                    np.array(plotting_data["y"])
                    - target_passband_wavelength[target_no][1]
                )
            )
