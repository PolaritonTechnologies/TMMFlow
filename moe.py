...
def f_L(q, a_1, a_2, a_3):
    ...
    L = (
        sp.power(q + a_1**2, -1.5)
        * sp.power(q + a_2**2, -0.5)
        * sp.power(q + a_3**2, -0.5)
    )
...
def m_D(a_x, a_y, a_z):
    ...
    D_x_int, err = sp_i.nquad(
        lambda t, z: f_Dx(t, z, a_x, a_y, a_z), [[0, 2.0 * np.pi], [0.0, 1.0]]
    )
    D_y_int, err = sp_i.nquad(
        lambda t, z: f_Dy(t, z, a_x, a_y, a_z), [[0, 2.0 * np.pi], [0.0, 1.0]]
    )
    D_z_int, err = sp_i.nquad(
        lambda t, z: f_Dz(t, z, a_x, a_y, a_z), [[0, 2.0 * np.pi], [0.0, 1.0]]
    )
