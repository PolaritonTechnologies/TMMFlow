...
def generate_eps_db(path_to_eps, ext="*.edb"):
    ...
    eps_dict = {"eps_files": eps_files, "eps_names": eps_names, "eps_db": eps_db}
...
def db_to_eps(wl, eps_db, eps_list):
    ...
    for eps in eps_list:
        if wl < eps_db[eps][0].min():
            raise ValueError(
                "interpolation wavelength too small: "
                + eps
                + "("
                + str(wl)
                + "/"
        ...
        if wl > eps_db[eps][0].max():
            raise ValueError(
                "interpolation wavelength too large: "
                + eps
                + "("
                + str(wl)
                + "/"
    ...
    e_list = np.array(
        [
            np.interp(wl, eps_db[eps][0], eps_db[eps][1])
...
def eps_corr_drude(wl, w_p, gamma_inf, vf, r):
    ...
    eps_r = (w_p**2) / (w**2 + 1j * w * gamma_inf) - (w_p**2) / (
        w**2 + 1j * w * gamma_r
    )
