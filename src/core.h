...
std::tuple<double, double, Matrix2cd> calculate_rt_coherent(std::vector<Matrix3cd> e_list_3x3, std::vector<double> d_list, const char* polarization, double wavelength, double theta_0, double phi_0)
{
    ...
    for (int i = 1; i < d_list.size() ; ++i)
    {
        ...
        if (strcmp(polarization, "p") == 0)
        {
            ...
            // Anisotropic dynamical matrix
            dynamical(0,0) = std::complex<double>(1,0) + quotient_dynamical;
...
std::tuple<double, double> calculate_rt(std::vector<Matrix3cd> e_list_3x3, std::vector<double> d_list, const char* polarization, double wavelength, double theta_0, double phi_0, double n_exit_medium)
{
    ...
    std::complex<double> transmission_coeff_reverse = total_Matrix.determinant() / total_Matrix(0,0);
    ...
    if (strcmp(polarization, "s") == 0)
    {
        transmissivity_no_backside_reverse = ((n_0.real() * std::sqrt(n_0.real() * (1 - std::sqrt(sin(theta_0) * sin(theta_0))))) / (n_s.real() * std::sqrt(1 - ((n_0.real() * n_0.real()) / (n_s.real() * n_s.real()) * sin(theta_0) * sin(theta_0))))) * ((transmission_coeff_reverse * std::conj(transmission_coeff_reverse)).real());
        r_subs_exit = (n_s.real() * cos_theta_sub - n_exit_medium * std::cos(theta_0)) / (n_s.real() * cos_theta_sub + n_exit_medium * std::cos(theta_0));
    }
    if (strcmp(polarization, "p") == 0)
    {
        // There could be a case in the future where we will take the conjugate of the first term according to Emmanuele Centurioni
        transmissivity_no_backside_reverse = ((n_0.real() * std::sqrt(n_0.real() * (1 - std::sqrt(sin(theta_0) * sin(theta_0))))) / (n_s.real() * std::sqrt(1 - ((n_0.real() * n_0.real()) / (n_s.real() * n_s.real()) * sin(theta_0) * sin(theta_0))))) * ((transmission_coeff_reverse * std::conj(transmission_coeff_reverse)).real());
    ...
    double T_subs_exit = 1 - R_subs_exit;
    return {reflectivity_coherent + (transmissivity_coherent * transmissivity_no_backside_reverse * R_subs_exit * std::pow(attenuation, 4)) / (1 - reflectivity_no_backside_reverse * R_subs_exit * std::pow(attenuation, 4)), (transmissivity_coherent * T_subs_exit * std::pow(attenuation, 2)) / (1 - reflectivity_no_backside_reverse * R_subs_exit * std::pow(attenuation, 4))};
};
