...
def execute_test(json_path, test_csv):
    ...
    filter_stack.calculate_ar_data(target_type="t")
    calculated_data = filter_stack.stored_data[0]
    calculated_data.columns = calculated_data.columns.astype(float)
    ...
    try:
        with open(test_csv, "r") as file:
            # load it with pandas
            test_reference = pd.read_csv(file, index_col=0)
            test_reference.columns = test_reference.columns.astype(float)
    ...
    test_reference = test_reference.apply(pd.to_numeric, errors="coerce")
    calculated_data = calculated_data.apply(pd.to_numeric, errors="coerce")

    comparison = np.isclose(test_reference, calculated_data, atol=1e-2)
    if comparison.all():
...
if __name__ == "__main__":
    ...
    json_paths = [
        ...
        "./src/tests/best-39layers-AlPcCl-MgF2-860nm-885nm.json",
    ]

    # Compare the performance of the current test to the reference test (csv file)
    test_csvs = [
        # "./src/tests/test_reference.csv",
        "./src/tests/best-39layers-AlPcCl-MgF2-860nm-885nm_reference.csv",
    ]
